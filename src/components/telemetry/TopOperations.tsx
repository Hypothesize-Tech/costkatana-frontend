...
export const TopOperations: React.FC = () => {
    ...
    return (
        <div className="glass rounded-xl p-8 border border-success-200/30 shadow-lg backdrop-blur-xl">
            <div className="flex justify-between items-center mb-6">
                ...
                <div className="glass rounded-lg border border-primary-200/30 p-1">
                    {['1h', '24h', '7d'].map((frame) => (
                        <button key={frame} onClick={() => setTimeframe(frame)} className={`px-4 py-2 font-display font-medium rounded-md transition-all duration-200 ${timeframe === frame ? 'bg-gradient-success text-white shadow-lg glow-success' : 'text-light-text-secondary dark:text-dark-text-secondary hover:bg-gradient-success/10'}`}>{frame}</button>
                    ))}
                </div>
            </div>
                ...
                <table className="w-full">
                    <thead>
                        <tr className="border-b border-primary-200/30">
                            ...
                            <th className="px-4 py-3 text-left font-display font-semibold gradient-text">Avg Latency (ms)</th>
                            <th className="px-4 py-3 text-left font-display font-semibold gradient-text">Error Rate</th>
                        </tr>
                    </thead>
                    <tbody>
                        {ops.map((op, i) => (
                            <tr key={`${op.name}-${i}`} className="border-b border-primary-200/20 hover:bg-gradient-success/5 transition-all duration-200">
                                ...
                                <td className="px-4 py-3 font-display font-semibold gradient-text-warning">{Number(op.avg_duration_ms || 0).toFixed(1)}</td>
