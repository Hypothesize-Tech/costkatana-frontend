                ...
                <div className="p-1 rounded-lg border shadow-lg backdrop-blur-xl glass border-primary-200/30">
                    {timeframes.map((frame) => (
                        <button
                            ...
                            className={`btn px-4 py-2 font-display font-medium rounded-md transition-all duration-200 ${timeframe === frame ? 'bg-gradient-primary text-white shadow-lg' : 'text-secondary-600 dark:text-secondary-300 hover:bg-gradient-primary/10'}`}
                        >
                            {frame}
                        </button>
                    ))}
            ...
            {chartData.length === 0 ? (
                ...
                <div className="grid grid-cols-1 gap-6 md:grid-cols-2">
                    ...
                    <div className="">
                        ...
                        <div className="mt-6">
                            <h4 className="mb-4 font-semibold font-display gradient-text-secondary">Models</h4>
                            <div className="flex flex-wrap gap-3">
                                {chartData.map((m, index) => (
                                    <div key={`${m.display_name}-${index}`} className="glass rounded-lg p-2 border border-primary-200/30 shadow-lg backdrop-blur-xl flex items-center max-w-[180px]">
                                        <span className="inline-block mr-2 w-4 h-4 rounded-full shadow-lg" style={{ backgroundColor: COLORS[index % COLORS.length] }} />
                                        <span className="text-sm truncate font-body text-secondary-900 dark:text-white" title={m.model_name}>{m.display_name}</span>
                                    </div>
                                ))}
                            </div>
                        </div>
                    ...
                    <div className="p-6 rounded-xl border shadow-lg backdrop-blur-xl glass border-success-200/30 dark:border-success-500/20 bg-gradient-light-panel dark:bg-gradient-dark-panel">
                        <div className="flex justify-between items-center mb-6">
                            ...
                            <span className="text-3xl font-bold font-display gradient-text-success">${(totalCost || 0).toFixed(2)}</span>
                        </div>
                        <div className="overflow-x-auto">
                            <table className="w-full">
                                <thead>
                                    <tr className="border-b border-primary-200/30">
                                        <th className="px-4 py-3 font-semibold text-left font-display gradient-text-primary">Model</th>
                                        <th className="px-4 py-3 font-semibold text-left font-display gradient-text-success">Cost ($)</th>
                                        <th className="px-4 py-3 font-semibold text-left font-display gradient-text-secondary">Requests</th>
                                        <th className="px-4 py-3 font-semibold text-left font-display gradient-text-accent">% of Total</th>
                                    </tr>
                                </thead>
                                <tbody>
                                    {costs.map((model, index) => {
                                        ...
                                        return (
                                            <tr key={key} className="border-b transition-all duration-200 border-primary-200/20 hover:bg-gradient-primary/5">
                                                ...
                                                <td className="px-4 py-3 font-semibold font-display gradient-text-success">${(model.total_cost_usd || 0).toFixed(2)}</td>
                                                <td className="px-4 py-3 font-body text-secondary-900 dark:text-white">{model.request_count || 0}</td>
                                                <td className="px-4 py-3 font-semibold font-display gradient-text-accent">{pct}%</td>
                                            </tr>
                                        );
                                    })}
                                </tbody>
