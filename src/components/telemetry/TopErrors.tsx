        ...
        <div className="p-3 sm:p-4 md:p-6 lg:p-8 rounded-xl border shadow-xl backdrop-blur-xl glass border-danger-200/30 bg-gradient-light-panel dark:bg-gradient-dark-panel">
            <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-2 sm:gap-3 mb-4 sm:mb-5 md:mb-6">
                <div className="flex gap-2 sm:gap-3 items-center">
                    ...
                    <h2 className="text-lg sm:text-xl font-bold font-display gradient-text-danger">Top Errors</h2>
                </div>
                <div className="p-1 rounded-lg border shadow-lg backdrop-blur-xl glass border-primary-200/30 flex flex-wrap gap-1">
                    {['1h', '24h', '7d'].map((frame) => (
                        <button key={frame} onClick={() => setTimeframe(frame)} className={`btn px-2.5 sm:px-3 md:px-4 py-1.5 sm:py-2 text-xs sm:text-sm font-display font-medium rounded-md transition-all duration-200 ${timeframe === frame ? 'bg-gradient-danger text-white shadow-lg' : 'text-secondary-600 dark:text-secondary-300 hover:bg-gradient-danger/10'}`}>{frame}</button>
                    ))}
                </div>
            </div>
                ...
                <table className="w-full min-w-[500px]">
                    <thead>
                        <tr className="border-b border-primary-200/30">
                            <th className="px-2 sm:px-3 md:px-4 py-2 sm:py-3 text-xs sm:text-sm font-semibold text-left font-display gradient-text-primary">Type</th>
                            <th className="px-2 sm:px-3 md:px-4 py-2 sm:py-3 text-xs sm:text-sm font-semibold text-left font-display gradient-text-danger">Count</th>
                            <th className="px-2 sm:px-3 md:px-4 py-2 sm:py-3 text-xs sm:text-sm font-semibold text-left font-display gradient-text-secondary">Latest</th>
                        </tr>
                    </thead>
                    <tbody>
                        {errs.map((e, i) => (
                            <tr key={`${e.type}-${i}`} className="border-b transition-all duration-200 border-primary-200/20 hover:bg-gradient-danger/5">
                                <td className="px-2 sm:px-3 md:px-4 py-2 sm:py-3 text-xs sm:text-sm font-body text-secondary-900 dark:text-white truncate max-w-[200px]" title={e.type || 'Unknown'}>{e.type || 'Unknown'}</td>
                                <td className="px-2 sm:px-3 md:px-4 py-2 sm:py-3 text-xs sm:text-sm font-semibold font-display gradient-text-danger">{e.count}</td>
                                <td className="px-2 sm:px-3 md:px-4 py-2 sm:py-3 text-xs sm:text-sm font-body text-secondary-600 dark:text-secondary-300">{new Date(e.latest_occurrence).toLocaleString()}</td>
                            </tr>
                        ))}
                    </tbody>
                </table>
