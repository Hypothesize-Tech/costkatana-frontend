...
import { SparklesIcon, CheckIcon, ClipboardDocumentIcon } from '@heroicons/react/24/outline';
...
const OptimizationWidget: React.FC<OptimizationWidgetProps> = ({
    ...
    const optimizeMutation = useMutation({
        mutationFn: () => optimizationService.getOptimizationPreview({
            prompt,
            service,
            model,
        ...
        <div className="space-y-4">
            ...
            {optimizationResult && (
                <div className="space-y-4">
                    ...
                    <div className="p-3 bg-green-50 rounded-lg border border-green-200">
                        ...
                        {optimizationResult.improvementPercentage && (
                            <div className="mt-1 text-xs text-green-700">
                                {optimizationResult.improvementPercentage.toFixed(1)}% improvement
                            </div>
                        )}
                    </div>
                                    ...
                                    <div className="mt-2">
                                        <div className="flex justify-between items-center mb-1">
                                            ...
                                            <div className="flex space-x-1">
                                                <button
                                                    onClick={() => handleCopy(suggestion.optimizedPrompt, index)}
                                                ...
                                                {onApplyOptimization && (
                                                    <button
                                                        onClick={() => handleApply(suggestion.optimizedPrompt, suggestion)}
                    ...
                    {/* Applied Techniques */}
                    {optimizationResult.techniques && optimizationResult.techniques.length > 0 && (
                        <div>
                            <h4 className="text-sm font-medium text-gray-900 mb-2">Applied Techniques</h4>
                            <div className="flex flex-wrap gap-2">
                                {optimizationResult.techniques.map((technique: string, index: number) => (
                                    <span
                                        key={index}
