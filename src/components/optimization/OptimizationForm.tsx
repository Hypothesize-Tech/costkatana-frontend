        ...
        <div className="flex fixed inset-0 z-50 justify-center items-center p-4 bg-black bg-opacity-50">
            <div className="bg-white rounded-lg shadow-xl w-full max-w-4xl max-h-[90vh] overflow-hidden">
                <div className="flex justify-between items-center p-6 border-b border-gray-200">
                    <h2 className="text-xl font-semibold text-gray-900">AI Prompt Optimization</h2>
                    <button
                        onClick={onClose}
                ...
                <div className="flex h-[calc(90vh-theme(spacing.20))]">
                    {/* Form Section */}
                    <div className={`overflow-y-auto p-6 ${showPreview ? 'w-1/2' : 'w-full'}`}>
                        <div className="space-y-6">
                            {/* Quick Setup */}
                            <div className="p-4 bg-blue-50 rounded-lg border border-blue-200">
                                <h3 className="text-sm font-medium text-blue-900 mb-2">Quick Start</h3>
                                <p className="text-sm text-blue-700">
                                    Simply paste your prompt below and we'll automatically optimize it for cost and performance.
                            ...
                            <div>
                                ...
                                <p className="mt-2 text-sm text-gray-500">
                                    Our AI will automatically apply prompt compression, context trimming, and other optimization techniques.
                                </p>
                            </div>
                            ...
                            <div className="flex pt-6 space-x-4 border-t border-gray-200">
                                <button
                                    ...
                                    disabled={previewMutation.isPending || !formData.prompt.trim()}
                    ...
                    {showPreview && previewData && previewData.suggestions && (
                        <div className="w-1/2 p-6 bg-gray-50 border-l border-gray-200 overflow-y-auto">
                            <div className="flex justify-between items-center mb-4">
                                <h3 className="text-lg font-medium text-gray-900">Optimization Preview</h3>
                                <button
                            ...
                            <div className="mb-6 p-4 bg-green-50 rounded-lg border border-green-200">
                                <div className="flex items-center justify-between mb-2">
                                    <div>
                                        <p className="text-sm font-medium text-green-900">Estimated Savings</p>
                                ...
                                {previewData.improvementPercentage && (
                                    <div className="flex justify-between text-sm text-green-700">
                                        <span>Improvement: {previewData.improvementPercentage.toFixed(1)}%</span>
                                        {previewData.originalTokens && previewData.optimizedTokens && (
                                            <span>Tokens: {previewData.originalTokens} → {previewData.optimizedTokens}</span>
                                        )}
                                    </div>
                                )}
                            </div>
                            ...
                            <div className="mb-6">
                                ...
                                <div className="space-y-2">
                                    {(previewData.techniques || []).map((technique: string, index: number) => (
                            ...
                            <div>
                                ...
                                {previewData.suggestions && previewData.suggestions.length > 0 ? (
                                    <div className="space-y-4">
                                        {formatOptimizationSuggestions(previewData.suggestions).map((suggestion: any, index: number) => (
                                            <div key={index} className="p-4 bg-white rounded-lg border">
                                                ...
                                                {suggestion.impact && (
                                                    <div className="mt-2">
                                                        <span className={`inline-flex items-center px-2 py-1 rounded-full text-xs font-medium ${suggestion.impact === 'high'
                                    ...
                                    <div className="p-4 bg-yellow-50 rounded-lg border border-yellow-200">
                                        <p className="text-sm text-yellow-800">
                                            No specific suggestions available, but optimization has been applied to reduce costs.
                                        </p>
                                    </div>
                                )}
                            </div>
