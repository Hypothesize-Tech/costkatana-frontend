...
export const ChatMessage: React.FC<ChatMessageProps> = ({ message, type, onApply, onSuggestedQueryClick }) => {
    ...
    const renderVisualizationPreview = () => {
        ...
        if (vizType === 'stat-card') {
            ...
            return (
                <div className="mt-4 p-6 bg-gradient-to-br from-primary-500/10 via-primary-400/5 to-transparent rounded-xl border-2 border-primary-500/30 shadow-lg">
                    ...
                    <div className="text-4xl font-black gradient-text-primary mb-2">
                        {typeof value === 'number' ?
        ...
        if (Array.isArray(data) && data.length > 0) {
            ...
            if (vizType === 'line') {
                return (
                    <div className="mt-4 bg-light-panel dark:bg-dark-panel p-5 rounded-xl border-2 border-primary-200/30 dark:border-primary-500/20 shadow-lg">
                        <div className="text-sm font-bold text-light-text-primary dark:text-dark-text-primary mb-3">Trend Analysis</div>
            ...
            if (vizType === 'bar') {
                return (
                    <div className="mt-4 bg-light-panel dark:bg-dark-panel p-5 rounded-xl border-2 border-primary-200/30 dark:border-primary-500/20 shadow-lg">
                        <div className="text-sm font-bold text-light-text-primary dark:text-dark-text-primary mb-3">Comparison Analysis</div>
            ...
            if (vizType === 'pie') {
                return (
                    <div className="mt-4 bg-light-panel dark:bg-dark-panel p-5 rounded-xl border-2 border-primary-200/30 dark:border-primary-500/20 shadow-lg">
                        <div className="text-sm font-bold text-light-text-primary dark:text-dark-text-primary mb-3">Distribution Analysis</div>
                        <ResponsiveContainer width="100%" height={200}>
                            <PieChart>
                                <Pie
                                    data={data.slice(0, 6)}
            ...
            if (vizType === 'table') {
                ...
                return (
                    <div className="mt-4 bg-light-panel dark:bg-dark-panel p-5 rounded-xl border-2 border-primary-200/30 dark:border-primary-500/20 shadow-lg overflow-x-auto">
                        <div className="text-sm font-bold text-light-text-primary dark:text-dark-text-primary mb-3">Data Table</div>
        ...
        <div className={`flex ${type === 'user' ? 'justify-end' : 'justify-start'} mb-6 animate-fade-in`}>
            <div className={`max-w-[85%] ${type === 'user' ? 'order-2' : 'order-1'}`}>
                ...
                <div className="flex items-start gap-3">
                    {type === 'ai' && (
                        <div className="w-9 h-9 rounded-xl bg-gradient-primary flex items-center justify-center shadow-lg glow-primary flex-shrink-0">
                    ...
                    <div className="flex-1">
                        <div
                            className={`rounded-2xl shadow-lg ${type === 'user'
                                ? 'bg-gradient-primary text-white px-5 py-4'
                                : 'bg-light-panel dark:bg-dark-panel border-2 border-primary-200/30 dark:border-primary-500/20 px-5 py-4'
                                }`}
                        >
                            {/* Message Header for AI */}
                            {type === 'ai' && (
                                <div className="flex items-center gap-2 mb-3 pb-2 border-b border-primary-200/30 dark:border-primary-500/20">
                            ...
                            <p className={`text-base leading-relaxed whitespace-pre-wrap ${type === 'user'
                            ...
                            {type === 'ai' && (
                                <>
                                    <div className="flex items-center gap-3 mt-4 pt-4 border-t-2 border-primary-200/30 dark:border-primary-500/20">
                                        {message.visualization && onApply && (
                                            <button
                                                onClick={() => onApply(message.visualization, message.visualization?.data || [])}
                                                className="flex-1 px-4 py-2.5 text-sm font-bold bg-gradient-primary text-white rounded-xl hover:scale-105 active:scale-95 transition-all duration-300 glow-primary shadow-lg flex items-center justify-center gap-2"
                                            >
                                        ...
                                        <button
                                            onClick={handleCopy}
                                            className="p-2.5 text-light-text-secondary dark:text-dark-text-secondary hover:text-primary-500 transition-all duration-300 rounded-xl hover:bg-primary-500/10 hover:scale-110"
                                            title="Copy response"
                                        >
                                            {copied ? (
                                    ...
                                    {message.suggestedQueries && message.suggestedQueries.length > 0 && onSuggestedQueryClick && (
                                        <div className="mt-4 pt-4 border-t border-primary-200/30 dark:border-primary-500/20">
                                            <div className="text-xs font-bold text-primary-600 dark:text-primary-400 uppercase tracking-wider mb-3">
                                            ...
                                            <div className="flex flex-col gap-2">
                                                {message.suggestedQueries.map((query, idx) => (
                                                    <button
                                                        ...
                                                        className="group flex items-center gap-2 px-4 py-2.5 bg-light-surface dark:bg-dark-surface hover:bg-primary-500/10 dark:hover:bg-primary-500/10 rounded-lg transition-all duration-300 text-left border border-primary-200/30 dark:border-primary-500/20 hover:border-primary-500/50 hover:shadow-md"
                                                    >
                                                        <FiArrowRight className="w-4 h-4 text-primary-500 group-hover:translate-x-1 transition-transform" />
                                                        <span className="text-sm text-light-text-primary dark:text-dark-text-primary font-medium flex-1">
                                                            {query}
                                                        </span>
                                                    </button>
                                                ))}
                        ...
                        <div className={`flex items-center gap-2 mt-2 px-2 ${type === 'user' ? 'justify-end' : 'justify-start'
                            }`}>
                            {type === 'user' && (
                    ...
                    {type === 'user' && (
                        <div className="w-9 h-9 rounded-xl bg-gradient-to-br from-gray-400 to-gray-600 flex items-center justify-center shadow-lg flex-shrink-0">
