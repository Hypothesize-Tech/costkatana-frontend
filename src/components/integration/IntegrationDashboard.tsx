...
export const IntegrationDashboard: React.FC<IntegrationDashboardProps> = ({ projectId }) => {
    ...
    const getActivityIcon = (service: string) => {
        if (service === 'openai') return <CpuChipIcon className="w-5 h-5 text-green-600" />;
        if (service === 'aws-bedrock') return <BoltIcon className="w-5 h-5 text-orange-600" />;
        if (service === 'anthropic') return <CodeBracketIcon className="w-5 h-5 text-purple-600" />;
    ...
    const getServiceColor = (service: string) => {
        switch (service) {
            ...
            case 'aws-bedrock': return 'bg-orange-100 text-orange-800 dark:bg-orange-900/20 dark:text-orange-400';
            case 'anthropic': return 'bg-purple-100 text-purple-800 dark:bg-purple-900/20 dark:text-purple-400';
            case 'google-ai': return 'bg-blue-100 text-blue-800 dark:bg-blue-900/20 dark:text-blue-400';
                                    ...
                                    <div key={index} className="p-4 hover:bg-gray-50 dark:hover:bg-gray-700 transition-colors">
                                        <div className="flex items-center justify-between">
                                            <div className="flex items-center space-x-3">
                                                <div className={`p-2 rounded-lg ${getServiceColor(usage.service)} bg-opacity-10`}>
                                                ...
                                                <div className="flex-1 min-w-0">
                                                    ...
                                                    <div className="flex items-center gap-4 mt-2 text-xs text-gray-500 dark:text-gray-400">
                                                        <span>{usage.totalTokens?.toLocaleString() || '0'} tokens</span>
                        ...
                        <div className="p-6">
                            ...
                            <div className="space-y-4">
                                <div className="flex gap-2">
                                    <span className={`px-2 py-1 text-xs font-medium rounded-full ${getServiceColor(selectedActivity.service)}`}>
                                ...
                                <div className="grid grid-cols-3 gap-4 text-sm">
                                    <div>
                                        ...
                                        <div className="font-medium">{formatCurrency(selectedActivity.cost || 0)}</div>
                                    </div>
                                    <div>
                                        ...
                                        <div className="font-medium">
                                            {selectedActivity.totalTokens ?
                                                ...
                                                (selectedActivity.promptTokens && selectedActivity.completionTokens ?
                                                    (selectedActivity.promptTokens + selectedActivity.completionTokens).toLocaleString() :
                                                    'N/A'
                                                )
                                            }
                                        </div>
