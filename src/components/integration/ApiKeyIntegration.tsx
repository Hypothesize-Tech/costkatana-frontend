...
export const ApiKeyIntegration: React.FC<ApiKeyIntegrationProps> = ({ isOpen, onClose }) => {
    const [activeTab, setActiveTab] = useState<'setup' | 'usage' | 'examples'>('setup');
    ...
    const generateIntegrationCode = () => {
        const selectedApiKey = apiKeys?.find(key => key.permissions.includes('read'));
                    ...
                    <div className="space-y-6">
                        ...
                        <div className="p-4 bg-blue-50 rounded-lg dark:bg-blue-900/20">
                            ...
                            <p className="mt-1 text-sm text-blue-700 dark:text-blue-300">
                                Your dashboard API keys provide secure access to your project data, usage tracking, and analytics.
                            </p>
                        </div>
                        ...
                        <div>
                            ...
                            <div className="space-y-2">
                                {apiKeys?.map(apiKey => (
                                    <div key={apiKey.keyId} className="flex justify-between items-center p-3 bg-gray-50 rounded-lg dark:bg-gray-800">
                                        <div className="flex items-center">
                                            ...
                                            <div>
                                                ...
                                                <p className="text-sm text-gray-500 dark:text-gray-400">
                                                    {apiKey.maskedKey} • Permissions: {apiKey.permissions.join(', ')}
                                                </p>
                                            </div>
                                        </div>
                                ...
                                {(!apiKeys || apiKeys.length === 0) && (
                                    <div className="py-6 text-center text-gray-500 dark:text-gray-400">
                                        ...
                                        <p className="text-sm">Create API keys in Settings to enable integration</p>
                                    </div>
                                )}
                            </div>
                        </div>
                            ...
                            {selectedProject && (
                                <div className="p-3 mt-2 bg-green-50 rounded-lg dark:bg-green-900/20">
                                    ...
                                    <p className="mt-1 text-xs text-green-600 dark:text-green-400">
                                        Project ID: <code className="px-1 bg-green-100 rounded dark:bg-green-800">{selectedProject}</code>
                                    </p>
                                </div>
                            )}
                        </div>
                    ...
                    <div className="space-y-6">
                        <div>
                            ...
                            <div className="space-y-4">
                                {Object.entries(generateIntegrationCode()).map(([language, code]) => (
                                    <div key={language} className="rounded-lg border dark:border-gray-700">
                                        <div className="flex justify-between items-center px-4 py-2 bg-gray-50 border-b dark:bg-gray-800 dark:border-gray-700">
                                            ...
                                            >
                                                {copied === language ? <CheckIcon className="mr-1 w-4 h-4" /> : <DocumentDuplicateIcon className="mr-1 w-4 h-4" />}
                        ...
                        <div className="p-4 bg-gray-50 rounded-lg dark:bg-gray-800">
                            ...
                            <div className="space-y-2 text-sm">
                                <div>
                                    <code className="text-blue-600 dark:text-blue-400">DASHBOARD_API_KEY</code>
                                    <span className="text-gray-600 dark:text-gray-400"> = {apiKeys?.find(k => k.permissions.includes('read'))?.maskedKey || 'Create an API key in Settings'}</span>
                                </div>
                                <div>
                                    <code className="text-blue-600 dark:text-blue-400">DASHBOARD_URL</code>
                                    <span className="text-gray-600 dark:text-gray-400"> = {window.location.origin}/api</span>
                                </div>
                                <div>
                                    <code className="text-blue-600 dark:text-blue-400">PROJECT_ID</code>
                                    <span className="text-gray-600 dark:text-gray-400"> = {selectedProject || 'Select a project above'}</span>
                                </div>
                            </div>
                        </div>
                    </div>
