                ...
                <div className="flex-1">
                    ...
                    {isExpanded && tipData.context && (
                        <div className="mt-3 space-y-2 text-sm text-gray-500">
                            {tipData.context.currentTokens && (
                                <div>
                                    Current tokens: <span className="font-medium">{tipData.context.currentTokens.toLocaleString()}</span>
                                    {tipData.context.threshold && (
                                        <span> (threshold: {tipData.context.threshold.toLocaleString()})</span>
                                    )}
                                </div>
                            )}
                            {tipData.context.currentModel && tipData.context.suggestedModel && (
                                <div>
                                    Switch from <span className="font-medium">{tipData.context.currentModel}</span> to{' '}
                                    <span className="font-medium">{tipData.context.suggestedModel}</span>
                                </div>
                            )}
                        </div>
                    )}
                    ...
                    {tipData.tip.action && (
                        <div className="flex items-center mt-3 space-x-3">
                            ...
                            >
                                {tipData.tip.action.type === 'enable_feature' && 'Enable Feature'}
                                {tipData.tip.action.type === 'optimize_prompt' && 'Optimize Now'}
    ...
    if (position === 'floating') {
        return (
            <div className="fixed right-4 bottom-4 z-50 max-w-md animate-slide-up">
                <div className={`p-4 rounded-lg border-2 shadow-lg ${getPriorityColor()}`}>
                    {renderContent()}
                </div>
            </div>
        );
