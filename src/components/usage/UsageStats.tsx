  ...
  return (
    <div className="grid grid-cols-1 gap-4 sm:gap-6 sm:grid-cols-2 lg:grid-cols-4">
      {cards.map((card, index) => (
        <div
          key={index}
          className="p-4 sm:p-6 rounded-2xl border border-primary-200/30 dark:border-primary-500/20 shadow-xl backdrop-blur-xl transition-all duration-300 glass bg-gradient-light-panel dark:bg-gradient-dark-panel hover:shadow-2xl hover:scale-[1.02] [touch-action:manipulation]"
        >
          <div className="flex justify-between items-center mb-3 sm:mb-4">
            <div className={`p-2.5 sm:p-3 rounded-xl glass border border-primary-200/30 dark:border-primary-500/20 ${card.bgColor}`}>
              <card.icon
            ...
            {card.change !== undefined && card.change !== 0 && (
              <div
                className={`flex items-center text-xs sm:text-sm font-medium px-2 py-1 rounded-xl glass border border-primary-200/30 dark:border-primary-500/20 ${(card.change > 0 && !card.inverse) ||
          ...
          <div>
            <p className="text-xs sm:text-sm font-medium text-secondary-600 dark:text-secondary-300">{card.title}</p>
            <p className="mt-1 text-xl sm:text-2xl font-bold font-display gradient-text-primary">
              {card.value}
            </p>
