...
import { trainingService, TrainingCandidate } from '../../services/training.service';
...
export const TrainingCandidatesList: React.FC<TrainingCandidatesListProps> = ({
    ...
    const handleSelectAll = () => {
        if (selectedIds.size === candidates.length) {
            ...
            setSelectedIds(new Set(candidates.map(c => c.usageData.metadata?.requestId).filter(Boolean)));
        }
    };

    const formatCost = (cost: number) => {
        return cost < 0.01 ? `$${(cost * 1000).toFixed(2)}k` : `$${cost.toFixed(3)}`;
    };
    ...
    if (loading) {
        return (
            <div className="flex justify-center items-center py-12">
                <LoadingSpinner />
                <span className="ml-3 text-gray-600">Loading training candidates...</span>
            </div>
        );
    }
        ...
        <div className="space-y-4">
                ...
                <div className="bg-blue-50 border border-blue-200 rounded-lg p-3">
                    <div className="flex items-center space-x-2">
                        ...
                        <span className="text-sm font-medium text-blue-900">
                            {selectedIds.size} request{selectedIds.size !== 1 ? 's' : ''} selected for training dataset
                        </span>
                    </div>
                </div>
            )}
            ...
            {candidates.length === 0 ? (
                <div className="text-center py-12 bg-gray-50 rounded-lg">
                    ...
                    <h3 className="text-lg font-medium text-gray-900 mb-2">No Training Candidates Found</h3>
                    <p className="text-gray-600 mb-4">
                        Start scoring your requests with 4+ stars to see training candidates here.
                    </p>
                </div>
            ) : (
                        ...
                        return (
                            <div
                                ...
                                className={`border rounded-lg p-4 transition-colors ${selectionMode
                                        ? `cursor-pointer hover:bg-gray-50 ${isSelected ? 'border-blue-500 bg-blue-50' : 'border-gray-200'
                                        }`
                                        : 'border-gray-200'
                                    }`}
                            ...
                            >
                                <div className="flex items-start justify-between">
                                    <div className="flex-1 min-w-0">
                                        ...
                                        <div className="mb-3">
                                            <p className="text-sm text-gray-900 line-clamp-2">
                                                <span className="font-medium">Prompt:</span> {candidate.usageData.prompt}
                                            </p>
                                            {candidate.usageData.completion && (
                                                <p className="text-sm text-gray-600 line-clamp-2 mt-1">
                                                    <span className="font-medium">Response:</span> {candidate.usageData.completion}
                                                </p>
                                            )}
                                        </div>
                                        ...
                                        <div className="flex items-center space-x-4 text-xs text-gray-500">
                                            <div className="flex items-center space-x-1">
                                                <span className="font-medium">{candidate.usageData.service}</span>
                                                <span>•</span>
                                            ...
                                            <div className="flex items-center space-x-1">
                                                ...
                                                <span>{formatTokens(candidate.usageData.totalTokens)} tokens</span>
                                            </div>
                                            ...
                                            <div className="flex items-center space-x-1">
                                                ...
                                                <span>{new Date(candidate.usageData.createdAt).toLocaleDateString()}</span>
                                            </div>
                                        </div>
                                        ...
                                        {candidate.requestScore.notes && (
                                            <div className="mt-2 p-2 bg-gray-50 rounded text-xs text-gray-700">
                                                <span className="font-medium">Notes:</span> {candidate.requestScore.notes}
                                            </div>
                                        )}
                                    </div>
                                    ...
                                    {/* Selection Indicator */}
                                    {selectionMode && (
                                        <div className={`ml-4 flex-shrink-0 w-5 h-5 rounded border-2 flex items-center justify-center ${isSelected
                                ...
                                <div className="mt-3 pt-3 border-t border-gray-100">
                                    <div className="flex justify-between text-xs text-gray-600">
                                        <span>
                                            Token Efficiency: <span className="font-medium">
                                                {candidate.requestScore.tokenEfficiency.toFixed(4)} score/token
                                            </span>
                                        </span>
                                        <span>
                                            Cost Efficiency: <span className="font-medium">
                                                {candidate.requestScore.costEfficiency.toFixed(2)} score/$
                                            </span>
                                        </span>
                                    </div>
                                </div>
