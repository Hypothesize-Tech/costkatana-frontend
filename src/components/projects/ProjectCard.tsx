...
export const ProjectCard: React.FC<ProjectCardProps> = ({
    ...
    const getBudgetColor = () => {
        if (isOverBudget) return 'text-red-600 bg-red-100 dark:bg-red-900/20 dark:text-red-200';
        if (isNearLimit) return 'text-yellow-600 bg-yellow-100 dark:bg-yellow-900/20 dark:text-yellow-200';
            ...
            <div className="p-6">
                ...
                <div className="mb-4">
                    <div className="flex justify-between items-center mb-2">
                        ...
                        <span className={`px-2 py-1 text-xs font-medium rounded-full ${getBudgetColor()}`}>
                            {budgetUsagePercentage.toFixed(0)}%
                            {isOverBudget && <FiAlertCircle className="inline ml-1 w-3 h-3" />}
                        </span>
                    </div>
