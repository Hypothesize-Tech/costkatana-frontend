...
import { AgentTraceDashboardShimmer } from '../shimmer/AgentTraceDashboardShimmer';
...
interface DashboardData {
  ...
  recentExecutions: TraceExecution[];
  performanceMetrics: {
...
interface ObservabilityDashboardApiResponse {
  overview: {
    totalExecutions: number;
    successRate: number;
    averageDuration: number;
...
const AgentTraceDashboard: React.FC = () => {
  ...
  const fetchDashboardData = useCallback(async () => {
    try {
      setRefreshing(true);
      const { agentTraceService } = await import('../../services/agentTrace.service');
      ...
      const transformedData: DashboardData = {
        ...
        recentExecutions: data.recentExecutions.map((exec: { traceId: string; traceName: string; totalCost: number; totalTokens: number; requestCount: number; averageCost: number; startTime: string; endTime: string; duration: number; steps: Array<{ step: string; sequence: number; cost: number; tokens: number; responseTime: number; model: string; service: string; timestamp: string }> }) => ({
          ...
          steps: exec.steps.map((step: { step: string; sequence: number; cost: number; tokens: number; responseTime: number; model: string; service: string; timestamp: string }) => ({
            id: step.step,
            name: step.step,
  ...
  if (loading || tabLoading[activeTab]) {
    return <AgentTraceDashboardShimmer activeTab={activeTab} />;
  }
        ...
        <div className="flex flex-col gap-3 justify-between items-start sm:flex-row sm:items-center sm:gap-4">
          <div className="flex gap-2 items-center sm:gap-3 md:gap-4 w-full sm:w-auto">
            ...
            <div className="min-w-0 flex-1 sm:flex-none">
              <h1 className="mb-0.5 text-lg font-bold font-display sm:mb-1 sm:text-xl md:mb-2 md:text-2xl lg:text-3xl gradient-text-primary break-words">Agent Trace Observatory</h1>
              <p className="text-xs font-body sm:text-sm md:text-base text-secondary-600 dark:text-secondary-300 break-words">Monitor, trace, and optimize your agent executions</p>
            </div>
          </div>
          <button
      ...
      <div className="rounded-xl border shadow-xl backdrop-blur-xl glass border-primary-200/30 dark:border-primary-500/20 bg-gradient-light-panel dark:bg-gradient-dark-panel md:rounded-2xl">
        ...
        <div className="p-3 sm:p-4 md:p-6">
            ...
            <div className="space-y-3 sm:space-y-4 md:space-y-6">
              ...
              <div className="grid grid-cols-1 gap-3 sm:grid-cols-2 md:gap-4 lg:grid-cols-3 lg:gap-6 xl:grid-cols-5">
                ...
                <div className="p-3 glass rounded-lg border border-highlight-200/30 dark:border-highlight-500/20 shadow-xl backdrop-blur-xl bg-gradient-to-br from-highlight-50/30 to-highlight-100/30 dark:from-highlight-900/20 dark:to-highlight-800/20 hover:scale-[1.02] transition-transform duration-300 [touch-action:manipulation] sm:p-4 md:p-6 md:rounded-xl">
                  <div className="flex items-center">
                    ...
                    <div className="ml-2 min-w-0 sm:ml-3 md:ml-4">
                      <p className="text-xs font-medium font-body text-highlight-600 dark:text-highlight-400 sm:text-sm">Active Traces</p>
                      <p className="text-lg font-bold font-display text-highlight-900 dark:text-highlight-100 sm:text-xl md:text-2xl">{dashboardData.overview.activeTraces}</p>
                    </div>
                  </div>
                </div>
              </div>
              ...
              <div className="grid grid-cols-1 gap-3 sm:gap-4 md:gap-6 lg:grid-cols-2">
                ...
                <div className="p-3 rounded-lg border shadow-xl backdrop-blur-xl glass border-primary-200/30 dark:border-primary-500/20 bg-gradient-light-panel dark:bg-gradient-dark-panel sm:p-4 md:p-6 md:rounded-xl">
                  <h3 className="mb-2 text-sm font-semibold font-display sm:mb-3 sm:text-base md:mb-4 md:text-lg text-secondary-900 dark:text-white">Cost Distribution by Trace</h3>
              ...
              {/* Recent Agent Trace Activity */}
              <div className="p-3 rounded-lg border shadow-xl backdrop-blur-xl glass border-primary-200/30 dark:border-primary-500/20 bg-gradient-light-panel dark:bg-gradient-dark-panel sm:p-4 md:p-6 md:rounded-xl">
                <h3 className="mb-2 text-sm font-semibold font-display sm:mb-3 sm:text-base md:mb-4 md:text-lg text-secondary-900 dark:text-white">Recent Agent Trace Activity</h3>
                {/* Mobile Card View */}
                <div className="block md:hidden space-y-2">
                  {dashboardData.recentExecutions.slice(0, 5).map((execution) => (
                    <div key={execution.id} className="p-3 rounded-lg border glass border-primary-200/30 dark:border-primary-500/20 bg-light-bg-100/50 dark:bg-dark-bg-100/50">
                      <div className="font-medium text-xs font-display text-secondary-900 dark:text-white mb-2 truncate">{execution.traceName}</div>
                ...
                <div className="hidden md:block overflow-x-auto rounded-lg border glass border-primary-200/30 dark:border-primary-500/20">
                  <table className="min-w-full divide-y divide-primary-200/20 dark:divide-primary-700/20">
                    <thead className="bg-gradient-to-r rounded-lg border glass border-primary-200/20 dark:border-primary-700/20 from-light-bg-300/50 to-light-bg-400/50 dark:from-dark-bg-300/50 dark:to-dark-bg-400/50">
                      <tr>
                        <th className="px-3 py-3 text-xs font-medium tracking-wider text-left uppercase sm:px-6 font-display text-secondary-500 dark:text-secondary-400">Trace</th>
                    ...
                    <tbody className="bg-gradient-to-br divide-y glass from-light-bg-100/50 to-light-bg-200/50 dark:from-dark-bg-100/50 dark:to-dark-bg-200/50 divide-primary-200/20 dark:divide-primary-700/20">
                      {dashboardData.recentExecutions.slice(0, 5).map((execution) => (
                        <tr key={execution.id} className="hover:bg-gradient-to-r hover:from-[#06ec9e]/5 hover:to-emerald-500/5 dark:hover:from-[#06ec9e]/10 dark:hover:to-emerald-500/10 transition-colors duration-300">
                          <td className="px-3 py-4 text-xs font-medium whitespace-nowrap sm:px-6 sm:text-sm font-display text-secondary-900 dark:text-white">{execution.traceName}</td>
          ...
          {activeTab === 'executions' && (
            <div className="space-y-3 sm:space-y-4 md:space-y-6">
              <h3 className="text-sm font-semibold font-display sm:text-base md:text-lg text-light-text-primary dark:text-dark-text-primary">Recent Agent Trace Executions</h3>
          ...
          {activeTab === 'traces' && (
            <div className="space-y-3 sm:space-y-4 md:space-y-6">
              <h3 className="text-sm font-semibold font-display sm:text-base md:text-lg text-light-text-primary dark:text-dark-text-primary">Agent Trace Details</h3>
              ...
              {selectedExecution ? (
                      ...
                      <div className="p-3 rounded-lg border shadow-xl backdrop-blur-xl glass border-primary-200/30 dark:border-primary-500/20 bg-gradient-light-panel dark:bg-gradient-dark-panel sm:p-4 md:p-6 md:rounded-xl">
                        <div className="flex flex-col gap-2 justify-between items-start mb-3 sm:flex-row sm:items-center sm:gap-3 md:gap-4 md:mb-4">
                          <h4 className="text-sm font-medium font-display sm:text-base md:text-lg text-light-text-primary dark:text-dark-text-primary break-words">
                            <span className="block sm:inline">{execution.traceName}</span>
                ...
                <div className="p-4 text-center rounded-lg border glass border-primary-200/30 dark:border-primary-500/20 bg-light-bg-200 dark:bg-dark-bg-200 sm:p-6 md:p-8 md:rounded-xl">
                  <p className="text-xs font-body sm:text-sm md:text-base text-light-text-secondary dark:text-dark-text-secondary">Select an agent trace execution to view detailed trace information</p>
