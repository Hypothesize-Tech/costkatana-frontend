...
class AgentService {
     ...
     */
    async getConversationHistory(conversationId?: string, limit?: number): Promise<ConversationHistoryResponse> {
        try {
     ...
     */
    async streamQuery(queryData: AgentQuery): Promise<ReadableStream<Uint8Array> | null> {
        try {
            const response = await fetch(`${api.defaults.baseURL}${this.baseURL}/stream`, {
                method: 'POST',
                headers: {
     ...
     */
    async getAnalytics(query: string, options?: {
        timeRange?: {
            startDate?: string;
            endDate?: string;
     ...
     */
    async getOptimizations(query: string, options?: {
     ...
     */
    async getModelRecommendations(query: string, useCase?: {
     ...
     */
    async manageProject(action: string, projectData?: any): Promise<AgentResponse> {
        return this.query({
    ...
    quickActions = {
        createProject: (name: string, type: string, requirements?: any) => 
            this.manageProject(`Create a new ${type} project named "${name}"`, requirements),
        
        analyzeSpending: (timeRange?: string) => 
            this.getAnalytics(`Analyze my spending patterns${timeRange ? ` for ${timeRange}` : ''}`),
        
        findSavings: (minAmount?: number) => 
            this.getOptimizations(`Find cost optimization opportunities${minAmount ? ` with minimum savings of $${minAmount}` : ''}`),
        
        recommendModel: (useCase: string, priority?: string) => 
            this.getModelRecommendations(`Recommend the best AI model for ${useCase}${priority ? ` with ${priority} priority` : ''}`),
        
        getDashboard: () => 
            this.getAnalytics('Show me my dashboard analytics with key metrics and insights'),
        
        detectAnomalies: () => 
            this.getAnalytics('Detect any cost anomalies or unusual spending patterns'),
        
        compareModels: (models: string[]) => 
            this.getModelRecommendations(`Compare these models: ${models.join(', ')} and tell me which is best`),
        
        optimizeProject: (projectId: string) => 
            this.getOptimizations(`Analyze and optimize project ${projectId} for cost savings`)
    };
     ...
     */
    async exportConversation(conversationId: string, format: 'json' | 'csv' | 'txt' = 'json'): Promise<Blob | null> {
        try {
            const response = await api.get(`${this.baseURL}/conversations/${conversationId}/export`, {
                params: { format },
                responseType: 'blob'
            });
            return response.data;
