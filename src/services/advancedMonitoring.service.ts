...
export class AdvancedMonitoringService {
    ...
    static async getTagAnalytics(filters?: {
        ...
        try {
            const response = await analyticsApiClient.get('/tags/analytics', { params: filters });
            return response.data.data || [];
        } catch (error: any) {
            console.error('Tag analytics request failed:', error.message);
            if (error.code === 'ECONNABORTED') {
                throw new Error('Tag analytics request timed out. Please try with fewer tags or a shorter time range.');
            }
            throw error;
        }
    }
    ...
    static async getRealTimeMetrics(filters?: {
        ...
        try {
            const response = await analyticsApiClient.get('/tags/realtime', { params: filters });
            return response.data.data || [];
        } catch (error: any) {
            console.error('Real-time metrics request failed:', error.message);
            if (error.code === 'ECONNABORTED') {
                throw new Error('Real-time metrics request timed out. Please try again.');
            }
            throw error;
        }
    }
    ...
    static async generateCostForecast(params: {
        ...
        try {
            const response = await longRunningApiClient.post('/forecasting/generate', params);
            return response.data.data;
        } catch (error: any) {
            console.error('Cost forecast request failed:', error.message);
            if (error.code === 'ECONNABORTED') {
                throw new Error('Cost forecast request timed out. Please try with a smaller time horizon.');
            }
            throw error;
        }
    }
    ...
    static async getPredictiveAlerts(params: {
        ...
        try {
            const response = await longRunningApiClient.post('/forecasting/alerts', params);
            return response.data.data || [];
        } catch (error: any) {
            console.error('Predictive alerts request failed:', error.message);
            if (error.code === 'ECONNABORTED') {
                throw new Error('Predictive alerts request timed out. Please try again.');
            }
            throw error;
        }
    }
    ...
    static async getSpendingPatterns(params: {
        ...
        try {
            const response = await analyticsApiClient.get('/forecasting/patterns', { params });
            return response.data.data || [];
        } catch (error: any) {
            console.error('Spending patterns request failed:', error.message);
            if (error.code === 'ECONNABORTED') {
                throw new Error('Spending patterns request timed out. Please try with a shorter time range.');
            }
            throw error;
        }
    }
    ...
    static async analyzeCostPerformanceCorrelation(params: {
        ...
        try {
            const response = await analyticsApiClient.post('/performance-cost/analyze', params);
            return response.data.data?.correlations || [];
        } catch (error: any) {
            console.error('Performance correlation request failed:', error.message);
            if (error.code === 'ECONNABORTED') {
                throw new Error('Performance analysis request timed out. Please try with a smaller date range.');
            }
            throw error;
        }
    }
    ...
    static async compareServices(params: {
        ...
        try {
            const response = await analyticsApiClient.post('/performance-cost/compare', params);
            return response.data.data || {};
        } catch (error: any) {
            console.error('Service comparison request failed:', error.message);
            if (error.code === 'ECONNABORTED') {
                throw new Error('Service comparison request timed out. Please try with fewer services.');
            }
            throw error;
        }
    }
    ...
    static async getPerformanceTrends(params: {
        ...
        try {
            const response = await analyticsApiClient.get('/performance-cost/trends', { params });
            return response.data.data || [];
        } catch (error: any) {
            console.error('Performance trends request failed:', error.message);
            if (error.code === 'ECONNABORTED') {
                throw new Error('Performance trends request timed out. Please try with a shorter time range.');
            }
            throw error;
        }
    }
    ...
    static async getOptimizationOpportunities(params: {
        ...
        try {
            const response = await analyticsApiClient.get('/performance-cost/opportunities', { params });
            return response.data.data || [];
        } catch (error: any) {
            console.error('Optimization opportunities request failed:', error.message);
            if (error.code === 'ECONNABORTED') {
                throw new Error('Optimization opportunities request timed out. Please try again.');
            }
            throw error;
        }
    }
    ...
    static async compareTags(params: {
        ...
        try {
            ...
            if (error.code === 'ECONNABORTED') {
                throw new Error('Tag comparison request timed out. Please try with fewer tags.');
            }
            throw error;
        }
    }
