...
class ExportService {
    ...
    async exportAnalytics(options: ExportOptions & {
        ...
        return this.processExportResponse(response.data, options.format, 'analytics');
    }

    async exportOptimizations(options: ExportOptions & {
        applied?: boolean;
        minSavings?: number;
    }): Promise<Blob> {
        ...
        return this.processExportResponse(response.data, options.format, 'optimizations');
    }

    async exportReport(type: 'daily' | 'weekly' | 'monthly' | 'custom', options?: {
        startDate?: string;
        endDate?: string;
        format?: ExportFormat;
        includeCharts?: boolean;
    ...
    async exportInvoice(invoiceId: string, format: 'pdf' | 'csv' = 'pdf'): Promise<Blob> {
        const response = await api.get(`/export/invoice/${invoiceId}`, {
            params: { format },
            responseType: 'blob',
        });
        return this.processExportResponse(response.data, format, `invoice-${invoiceId}`);
    }
