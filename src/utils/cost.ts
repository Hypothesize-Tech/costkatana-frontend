...
export function calculateCost(
    ...
    if (!pricing) {
        ...
        if (model.toLowerCase().includes('grok-4')) {
            ...
            pricing = PRICING_DATA.find(p => p.model === 'claude-sonnet-4-20250514');
        } else if (model.toLowerCase().includes('claude-3-haiku')) {
            pricing = PRICING_DATA.find(p => p.model === 'claude-3-5-haiku-20241022');
...
export function compareModelCosts(
    ...
    const comparisons = modelsToCompare.map(pricing => {
        const cost = calculateCost(inputTokens, outputTokens, pricing.provider, pricing.model);
